...
class KnowledgePacket:
    ...
    - coordinates: bounding-box coordinates for the detected object
    - detection_id: stable id for this specific detection event
    - image: compressed full-frame payload for visual transfer/retraining
    - source_model: model/backbone that produced the detection
    ...
    def to_dict(self, include_embedding_as_list: bool = False) -> Dict[str, Any]:
        ...
        if self.source_model is not None:
            d["source_model"] = self.source_model
        if self.best_model is not None:
            d["best_model"] = self.best_model
        if self.best_confidence is not None:
            d["best_confidence"] = float(self.best_confidence)
        if self.verified_by is not None:
            d["verified_by"] = self.verified_by
        if self.agreement_count is not None:
            d["agreement_count"] = int(self.agreement_count)
    ...
    def from_dict(data: Dict[str, Any]) -> "KnowledgePacket":
        ...
        return KnowledgePacket(
            ...
            agreement_count=data.get("agreement_count"),
        )
