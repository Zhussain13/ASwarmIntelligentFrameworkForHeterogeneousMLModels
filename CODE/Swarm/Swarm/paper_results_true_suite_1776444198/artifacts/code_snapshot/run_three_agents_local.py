...
def _run_heavy_inference(detector_mode: str, frame: np.ndarray, cfg: Config, detector_obj) -> Tuple[str, float, float]:
    t0 = time.perf_counter()
    if detector_mode == "simulated":
        time.sleep(cfg.heavy_inference_ms / 1000.0)
        dt_ms = (time.perf_counter() - t0) * 1000.0
        return "Person", 0.90, dt_ms
    ...
    if len(rects) > 0:
        ...
        return "Person", conf, dt_ms
    return "Background", 0.60, dt_ms
    ...
    try:
        for frame_idx, img in enumerate(frames, start=1):
            ...
            if best_pkt is not None:
                ...
                decision = "heavy"
                heavy_label, heavy_confidence, heavy_latency_ms = _run_heavy_inference(
                    detector_mode,
                    img,
                    cfg,
                    detector_obj,
